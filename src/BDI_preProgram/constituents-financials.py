...
#############################################################################
#	All source files in the current directory are taken and modified from this
...
reader = csv.reader(open('../../input_files/constituents.csv', 'r'))
...
for idx in range(0,len(symbols),20):
    ...
    fo_b = request.urlopen(query)
    fo = fo_b.read().decode('utf-8')

    rows = fo.rstrip('\n').split('\n')
    for count, row in enumerate(rows):
        rows[count] = row.split(',')
...
fo = open('../../input_files/constituents-financials.csv', 'w')
